...
type FontDecorationInput = 'none' | 'underline' | 'striketrough' | 'overline' | FontDecoration;
...
function toStrokeWidth(input: StrokeWidthInput | undefined): StrokeWidth | undefined {
  if (input == null) {
    return undefined;
  }
...
function toCornerRadius(input: CornerRadiusInput | undefined): CornerRadius | undefined {
  if (input == null) {
    return undefined;
  }
...
function toStrokeAlign(input: StrokeAlignInput | undefined): StrokeAlign | undefined {
  if (input == null) {
    return undefined;
  }
...
function toLineEnding(input: LineEndingInput | undefined): LineEnding | undefined {
  if (input == null) {
    return undefined;
  }
...
function toTextVerticalAlign(input: TextVerticalAlignInput | undefined): TextVerticalAlign | undefined {
  if (input == null) {
    return undefined;
  }
...
function toTextWrapMode(input: TextWrapModeInput | undefined): TextWrapMode | undefined {
  if (input == null) {
    return undefined;
  }
...
function toFontDecoration(input: FontDecorationInput | undefined): FontDecoration | undefined {
  if (input == null) {
    return undefined;
  }
...
function toUnderlineStyle(
  ...
  return underlineStyleMap[String(input)] ?? FontDecorationUnderlineStyle.Solid;
}
...
function applyCommonShapeProps(node: ShapeNodeLike, nodeSpec: ShapeNodePreviewBase): void {
  ...
  if (typeof nodeSpec.scaleX === 'number' || typeof nodeSpec.scaleY === 'number') {
...
function applyCommonLineProps(node: LineNodeLike, nodeSpec: LineNodePreviewSpec): void {
  node.setPosition(nodeSpec.x ?? 0, nodeSpec.y ?? 0);
...
function applyCommonPathProps(node: PathNodeLike, nodeSpec: PathNodePreviewSpec): void {
  node.setPosition(nodeSpec.x ?? 0, nodeSpec.y ?? 0);

  if (typeof nodeSpec.width === 'number' && typeof nodeSpec.height === 'number') {
  ...
  } else if (typeof nodeSpec.svgPath === 'string' && nodeSpec.svgPath.trim().length > 0) {
...
function applyCommonImageProps(node: ImageNodeLike, nodeSpec: ImageNodeLikeSpec): void {
...
function applyCommonVideoProps(node: VideoNodeLike, nodeSpec: VideoNodePreviewSpec): void {
...
function applyCommonTextProps(node: TextNodeLike, nodeSpec: TextNodePreviewSpec): void {
  applyCommonShapeProps(node, nodeSpec);
...
type OverlayDebugNode = BoundsNode & {
  ...
  getWorldMatrix(): { a: number; b: number; c: number; d: number; tx: number; ty: number };
  getWidth(): number;
  getHeight(): number;
  getPivotX(): number;
  getPivotY(): number;
...
function addDebugOverlays(
  ...
  for (const node of nodes) {
    if (options.showWorldAABB) {
      ...
      aabbNode.setPosition(aabb.x + aabb.width / 2, aabb.y + aabb.height / 2);
...
function FlowscapeScenePreviewInner({
  ...
  const { showAABB, showOBB, showPivot, showViewBounds, showOrbit } = debugNodes;
  ...
  useEffect(() => {
    ...
    const createGroupNode = (groupSpec: GroupNodePreviewSpec): void => {
      ...
      if (typeof groupSpec.scaleX === 'number' || typeof groupSpec.scaleY === 'number') {
        groupNode.setScale(groupSpec.scaleX ?? 1, groupSpec.scaleY ?? 1);
      }
