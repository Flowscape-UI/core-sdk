...
export class HandleCornerRadius implements IHandleCornerRadius {
    ...
    private _getInsetDiagonalPoint(section: CornerRadiusSection): Point {
        const dirX = this._normalize({
            x: section.xAxisPoint.x - section.origin.x,
            y: section.xAxisPoint.y - section.origin.y,
        });

        const dirY = this._normalize({
            x: section.yAxisPoint.x - section.origin.x,
            y: section.yAxisPoint.y - section.origin.y,
        });

        return {
            x: section.origin.x + dirX.x * section.inset + dirY.x * section.inset,
            y: section.origin.y + dirX.y * section.inset + dirY.y * section.inset,
        };
    }
