...
export class RulerGuidesPlugin extends Plugin {
    ...
    protected onAttach(core: CoreEngine): void {
        ...
        world.on('xChange.ruler-guides yChange.ruler-guides scaleXChange.ruler-guides scaleYChange.ruler-guides', () => {
    ...
    private _updateGuidesPositions() {
        ...
        this._guides.forEach(guide => {
            ...
            if (worldCoord === undefined) return;

            const isHorizontal = guide.name() === 'guide-h';

            if (isHorizontal) {
                const screenY = world.y() + worldCoord * scale;
                guide.position({ x: 0, y: screenY });
                guide.points([0, 0, stage.width(), 0]);
            } else {
                const screenX = world.x() + worldCoord * scale;
                guide.position({ x: screenX, y: 0 });
                guide.points([0, 0, 0, stage.height()]);
            }
        });
    ...
    private _snapToGrid(coord: number): number {
     ...
     private _startCreateGuide(type: 'h' | 'v') {
        ...
        line.on('dragmove', () => {
            ...
            const rulerLayer = this._core.stage.findOne('.ruler-layer') as Konva.Layer | undefined;
            if (rulerLayer) {
