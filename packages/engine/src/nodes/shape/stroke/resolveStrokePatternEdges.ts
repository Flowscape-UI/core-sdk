...
export function resolveStrokePatternEdges(
	 ...
	 */
	const windingSign = winding < -EPSILON ? -1 : 1;

	const resolvedSegments: ResolvedStrokePatternEdgeSegment[] = [];
	...
	for (const segment of segments) {
		...
		for (let index = 1; index < segment.points.length; index += 1) {
			...
			const tangent = {
				x: MathF32.toF32(dx / length),

				y: MathF32.toF32(dy / length),
			};
			 ...
			 */
			const outwardNormal = {
				x: MathF32.toF32((windingSign * dy) / length),

				y: MathF32.toF32((-windingSign * dx) / length),
			};
			...
			const endDistance = Math.min(segment.end, startDistance + length);

			edges.push({
				start: clonePoint(start),
				end: clonePoint(end),

				startDistance: MathF32.toF32(startDistance),

				endDistance: MathF32.toF32(endDistance),

				length: MathF32.toF32(length),
