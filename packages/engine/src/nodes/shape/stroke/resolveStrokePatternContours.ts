...
export function resolveStrokePatternContours(
	...
	const resolvedSegments: ResolvedStrokePatternContourSegment[] = [];
	...
	for (const segment of segments) {
		...
		const outer = resolveOffsetContour(segment.edges, "outer");

		const inner = resolveOffsetContour(segment.edges, "inner");

		if (outer.length < 2 || inner.length < 2) {
			continue;
		}

		const firstEdge = segment.edges[0]!.source;

		const lastEdge = segment.edges[segment.edges.length - 1]!.source;
...
function resolveOffsetContour(
	...
	const points: Vector2[] = [
		clonePoint(
			side === "outer" ? firstEdge.outerStart : firstEdge.innerStart,
		),
	];
	...
	for (let index = 1; index < edges.length; index += 1) {
		...
		const previousStart =
			side === "outer" ? previous.outerStart : previous.innerStart;

		const previousEnd =
			side === "outer" ? previous.outerEnd : previous.innerEnd;

		const currentStart =
			side === "outer" ? current.outerStart : current.innerStart;

		const currentEnd =
			side === "outer" ? current.outerEnd : current.innerEnd;

		const previousDirection = {
			x: previousEnd.x - previousStart.x,

			y: previousEnd.y - previousStart.y,
		};
		...
		if (intersection) {
			appendUniquePoint(points, intersection);
		...
		appendUniquePoint(points, {
			x: (previousEnd.x + currentStart.x) * 0.5,

			y: (previousEnd.y + currentStart.y) * 0.5,
		});
	}
	...
	appendUniquePoint(
		points,
		side === "outer" ? lastEdge.outerEnd : lastEdge.innerEnd,
	);
...
function intersectLines(
	...
	return {
		x: MathF32.toF32(firstPoint.x + firstDirection.x * t),

		y: MathF32.toF32(firstPoint.y + firstDirection.y * t),
	};
}
