import { type Vector2, type ID, MathF32 } from "../../core";
import { type Rect, NodeBase, NodeType } from "../base";
import { ShapeBase } from "../shape";
import { type INodeGroup, GroupChildrenResizeMode } from "./types";
...
export class NodeGroup extends ShapeBase implements INodeGroup {
	...
	private _resizeToBounds(targetBounds: Rect): void {
		...
		this._applyResizeFromSnapshot(
			snapshot,
			targetBounds,
			GroupChildrenResizeMode.Scale,
		);
	}
	...
	private _applyResizeFromSnapshot(
		...
		const scaleX =
			sourceWidth <= GROUP_EPSILON
				? 0
				: MathF32.toF32(nextWidth / sourceWidth);

		const scaleY =
			sourceHeight <= GROUP_EPSILON
				? 0
		...
		for (const child of children) {
			...
			const rotated = this._isNodeRotatedInGroupAxes(
				childSnapshot.rotation,
			);
			const sourcePosition = childSnapshot.position;

			let nextPositionX = MathF32.add(
				targetX,
				MathF32.mul(MathF32.sub(sourcePosition.x, sourceX), scaleX),
			);

			let nextPositionY = MathF32.add(
				targetY,
				MathF32.mul(MathF32.sub(sourcePosition.y, sourceY), scaleY),
			);
