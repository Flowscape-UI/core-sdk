...
import { FontStyle, TextAlign, TextVerticalAlign, TextWrapMode } from "./types";
...
export class NodeText extends ShapeBase implements INodeText {
	...
	public override toPathCommands(): readonly ShapePathCommand[] {
		...
		const lineHeightPx = Math.max(
			1,
			this._fontSize * Math.max(0, this._lineHeight),
		);
	...
	private _wrapParagraphByWords(
		...
		for (const word of words) {
			...
			if (
				current.length > 0 &&
				this._measureTextWidth(candidate) > maxWidth
			) {
				lines.push(current);

				if (this._measureTextWidth(word) > maxWidth) {
					const splitWord = this._wrapParagraphByCharacters(
						word,
						maxWidth,
					);
	...
	private _wrapParagraphByCharacters(
		paragraph: string,
		maxWidth: number,
	): string[] {
		if (paragraph.length === 0) {
	...
	private _getFontShorthand(): string {
		const style =
			this._fontStyle === FontStyle.Normal ? "normal" : this._fontStyle;
