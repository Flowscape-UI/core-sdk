...
describe("ConicGradient", () => {
	...
	describe("fromStops()", () => {
		...
		it("should throw if less than 2 stops", () => {
			expect(() => ConicGradient.fromStops([{ color: "red" }])).toThrow();
		});
	});
	...
	describe("fromString()", () => {
		...
		it("should parse 'from [angle]' parameter", () => {
			const g = ConicGradient.fromString(
		...
		it("should parse 'at [position]' parameter", () => {
			const g = ConicGradient.fromString(
				"conic-gradient(at 10% 20%, red, blue)",
			);
		...
		it("should parse repeating-conic-gradient", () => {
			const g = ConicGradient.fromString(
				"repeating-conic-gradient(red, blue 10%)",
			);
		...
		it("should handle rgba/rgb colors with commas correctly", () => {
			const g = ConicGradient.fromString(
				"conic-gradient(rgba(255, 0, 0, 1), rgb(0, 0, 255))",
			);
	...
	describe("isValidString()", () => {
		it("should validate correct conic strings", () => {
			expect(
				ConicGradient.isValidString("conic-gradient(red, blue)"),
			).toBe(true);
			expect(
				ConicGradient.isValidString(
		...
		it("should reject invalid strings", () => {
			expect(
				ConicGradient.isValidString("linear-gradient(red, blue)"),
			).toBe(false);
			expect(ConicGradient.isValidString("conic-gradient(red)")).toBe(
				false,
			); // < 2 stops
		});
	});
