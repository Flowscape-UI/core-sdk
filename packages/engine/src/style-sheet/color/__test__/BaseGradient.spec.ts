...
describe("BaseGradient: _parseColorStops", () => {
	...
	describe("_parseColorStops()", () => {
		const getOffsets = (input: string) =>
			parser.parseColorStops(input).stops.map((s) => s.offset);

		it("auto spread (edge 0 -> 1)", () => {
			expect(getOffsets("red, blue")).toEqual([0, 1]);
			expect(getOffsets("red, green, blue")).toEqual([0, 0.5, 1]);
			expect(getOffsets("red, yellow, green, blue")).toEqual([
				0, 0.333, 0.667, 1,
			]);
		});
		...
		it("dificult spread (green, red, cyan 50%, velvet)", () => {
			expect(getOffsets("green, red, cyan 50%, purple")).toEqual([
				0, 0.25, 0.5, 1,
			]);
		});
		...
		it("working with large amount of colors (10 colors)", () => {
			const input =
				"red, green, blue, cyan, white, black, yellow, pink, orange, gray";
		...
		it("processing percents with floating point", () => {
			expect(getOffsets("red 10.5%, blue 20.333%")).toEqual([
				0.105, 0.203,
			]);
		});
	});
	...
	describe("Direction Parsing (Killer Feature)", () => {
		const getDirection = (input: string) =>
			parser.parseColorStops(input).direction;
		...
		it("combo check: direction + difficult colors", () => {
			const result = parser.parseColorStops(
				"to top left, rgba(255,0,0,0.5) 10%, blue 90%",
			);
	...
	describe("Multi-layer Parsing (_parseString)", () => {
		...
		it("should parse multiple gradient layers correctly", () => {
			const input =
				"gradient(to top, red, blue), gradient(135deg, green 10%, yellow 90%)";
		...
		it("should handle complex colors (rgba/rgb) within multiple layers", () => {
			const input =
				"gradient(rgba(255, 255, 255, 0.5) 0%, black 100%), gradient(to left, rgb(255,0,0), transparent)";
			const result = parser.parseString(input);
