...
export class ConicGradient {
	...
	public static fromString(input: string): ConicGradient {
		...
		if (
			params.length > 0 &&
			(params[0]!.includes("from ") || params[0]!.includes("at "))
		) {
	...
	public getAngle(): ConicGradientAngleInput {
		return this._angle;
	}
	...
	private static _parseOffset(
		value?: number | `${number}%` | `${number}deg`,
	): number {
	...
	private static _normalizeStops(
		...
		if (stops.length < 2)
			throw new Error("[ConicGradient] Need at least 2 stops");
		return stops
	...
	private static _fillMissingOffsets(
		...
		const out: Array<{ color: Color; offset: number }> = stops.map((s) => ({
			...s,
		}));
		...
		if (
			out[out.length - 1] &&
			!Number.isFinite(out[out.length - 1]!.offset)
		)
		...
		while (i < out.length) {
			...
			const startIdx = i - 1;
			let endIdx = i;
			while (endIdx < out.length && !Number.isFinite(out[endIdx]!.offset))
				endIdx++;
