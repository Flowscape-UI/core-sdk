...
export class RadialGradient {
	...
	private constructor(
		...
		opts: Required<Omit<RadialGradientOptions, "repeating">> & {
			repeating: boolean;
		},
	) {
	...
	public static fromString(input: string): RadialGradient {
		...
		if (!isRepeating && !isNormal) {
			throw new Error(
				`[RadialGradient] Expected radial-gradient but got: "${trimmed}"`,
			);
		}
		...
		if (start === -1 || end === -1)
			throw new Error("[RadialGradient] Invalid syntax");
		...
		const hasOptions = /circle|ellipse|at|closest|farthest/.test(
			firstParam,
		);
		...
		if (hasOptions) {
			...
			const extents: RadialGradientExtent[] = [
				"closest-side",
				"closest-corner",
				"farthest-side",
				"farthest-corner",
		...
		const stops = params.map((p) => {
			...
			if (isOffset) {
				const colorPart = item
					.slice(0, item.lastIndexOf(lastPart!))
					.trim();
				return {
	...
	private static _normalizeStops(
		stops: ColorStopCanonical[],
	): ColorStopCanonical[] {
		if (stops.length < 2)
			throw new Error("[RadialGradient] Need at least 2 stops");
	...
	private static _fillMissingOffsets(
		...
		if (!out.some((s) => Number.isFinite(s.offset))) {
			...
			if (!Number.isFinite(out[out.length - 1]!.offset))
			...
			for (let i = 1; i < out.length - 1; i++) {
				if (!Number.isFinite(out[i]!.offset)) {
					...
					while (
						nextIdx < out.length &&
						!Number.isFinite(out[nextIdx]!.offset)
					)
						nextIdx++;
