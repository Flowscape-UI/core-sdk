...
import type { ColorStopCanonical, GradientOption } from "./types";
...
export abstract class BaseGradient implements IGradient {
	...
	protected static _parseColorStops(input: string): GradientOption {
		...
		if (parts.length > 0) {
			...
			const isNamedDirection =
				firstPart.startsWith("to ") &&
				this._directionsMap[firstPart] !== undefined;
		...
		for (const part of parts) {
			...
			if (percentMatches && percentMatches.length >= 2) {
				...
				const offset = percentMatches
					? Math.round((parseFloat(percentMatches[0]) / 100) * 1000) /
						1000
					: null;
		...
		if (intermediateStops[0]?.offset === null)
			intermediateStops[0].offset = 0;
		...
		for (let i = 0; i < intermediateStops.length; i++) {
			if (intermediateStops[i]!.offset === null) {
				...
				while (
					leftIdx >= 0 &&
					intermediateStops[leftIdx]!.offset === null
				)
					leftIdx--;

				let rightIdx = i + 1;
				while (
					rightIdx < intermediateStops.length &&
					intermediateStops[rightIdx]!.offset === null
				)
					rightIdx++;
				...
				const val =
					startOffset +
					(endOffset - startOffset) * ((i - leftIdx) / steps);
	...
	private static _extractGradientValues(input: string): string[] {
		...
		while (i < input.length) {
			...
			if (contentEnd !== -1) {
				...
				const content = input
					.substring(openBracketIdx + 1, contentEnd)
					.trim();
				values.push(content);
