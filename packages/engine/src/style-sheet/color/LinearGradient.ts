import { type ColorStopCanonical, type GradientOption } from "./types";
...
export class LinearGradient extends BaseGradient {
	...
	public static fromString(input: string) {
		...
		const markers = isRepeating
			? ["repeating-linear-gradient()"]
			: ["linear-gradient()"];
		const normalized = replaceMarkersFromInputString(
			input,
			markers,
			"gradient()",
		);
	...
	public static fromStops(
		...
		const originalStops = [...inputs.stops].sort(
