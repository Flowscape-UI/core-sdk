...
export class Input {
	...
	private static _onInputCallback: ((event: InputEventInfo) => void) | null =
		null;
	...
	public static onInput(
		callback: ((event: InputEventInfo) => void) | null,
	): void {
	...
	public static get ctrlPressed(): boolean {
		return (
			this._keys.has(KeyCode.LeftControl) ||
			this._keys.has(KeyCode.RightControl)
		);
	}
	public static get shiftPressed(): boolean {
		return (
			this._keys.has(KeyCode.LeftShift) ||
			this._keys.has(KeyCode.RightShift)
		);
	}
	public static get altPressed(): boolean {
		return (
			this._keys.has(KeyCode.LeftAlt) || this._keys.has(KeyCode.RightAlt)
		);
	}
	public static get metaPressed(): boolean {
		return (
			this._keys.has(KeyCode.LeftCommand) ||
			this._keys.has(KeyCode.RightCommand) ||
			this._keys.has(KeyCode.LeftWindows) ||
			this._keys.has(KeyCode.RightWindows)
		);
	}
	...
	public static get mousePositionDelta(): Vector2 {
	...
	public static get mouseScrollX(): number {
		return this._mouseScroll.x;
	}
	public static get mouseScrollY(): number {
		return this._mouseScroll.y;
	}
	public static get mouseInside(): boolean {
		return this._mouseInside;
	}
	public static get anyMouseButton(): boolean {
		return this._mouseButtons.size > 0;
	}
	public static get anyMouseButtonDown(): boolean {
		return this._mouseButtonsDown.size > 0;
	}

	// --- Scroll modifiers ---
	public static get scrollCtrl(): boolean {
		return this._scrollCtrl;
	}
	public static get scrollShift(): boolean {
		return this._scrollShift;
	}
	public static get scrollAlt(): boolean {
		return this._scrollAlt;
	}
	public static get scrollMeta(): boolean {
		return this._scrollMeta;
	}
	...
	public static get pointerDelta(): Vector2 {
		return { ...this._pointerDelta };
	}
	public static get pointerDown(): boolean {
		return this._pointerDown;
	}
	public static get pointerInside(): boolean {
		return this._pointerInside;
	}
	public static get pointerCaptured(): boolean {
		return this._pointerCaptured;
	}
	public static get pointerType(): "mouse" | "pen" | "touch" {
		return this._pointerType;
	}
	public static get pointerPressure(): number {
		return this._pointerPressure;
	}
	public static get activePointerId(): number | null {
	...
	public static pointerToSurfacePoint(
		...
		return this.clientToSurfacePoint(
			surface,
	...
	// --- Coalesced / Predicted ---
	public static get coalescedEvents(): ReadonlyArray<PointerEvent> {
		return this._coalescedEvents;
	}
	public static get predictedEvents(): ReadonlyArray<PointerEvent> {
	...
	public static getKeyDownCombo(...keys: KeyCode[]): boolean {
		return (
			keys.every((key) => this._keys.has(key)) &&
			keys.some((key) => this._keysDown.has(key))
		);
	}

	public static getKeyRepeat(
		key: KeyCode,
	 ...
	 */
	public static getMouseButtonClickCount(
		button: number,
	...
	public static getMouseButtonDoubleClick(
		button: number,
		threshold = 300,
	): boolean {
		return (
			this.getMouseButtonDown(button) &&
			this.getMouseButtonClickCount(button, threshold) === 2
		);
	}

	/** true именно в кадр когда зафиксирован тройной клик. */
	public static getMouseButtonTripleClick(
		button: number,
		threshold = 300,
	): boolean {
		return (
	...
	public static _registerSurface(surface: InputSurface): void {
		...
		surface.addEventListener(
			"mouseenter",
			this._onMouseEnter as EventListener,
		);
		surface.addEventListener(
			"mouseleave",
			this._onMouseLeave as EventListener,
		);
		surface.addEventListener("wheel", this._onWheel as EventListener, {
			passive: false,
		});
		surface.addEventListener(
			"contextmenu",
			this._onContextMenu as EventListener,
		);
		...
		surface.addEventListener(
			"pointerenter",
			this._onPointerEnter as EventListener,
		);
		surface.addEventListener(
			"pointerleave",
			this._onPointerLeave as EventListener,
		);
		surface.addEventListener(
			"pointerover",
			this._onPointerOver as EventListener,
		);
		surface.addEventListener(
			"pointerout",
			this._onPointerOut as EventListener,
		);
		surface.addEventListener(
			"gotpointercapture",
			this._onGotPointerCapture as EventListener,
		);
		surface.addEventListener(
			"lostpointercapture",
			this._onLostPointerCapture as EventListener,
		);
	...
	private static _onKeyDown = (e: KeyboardEvent): void => {
		...
		if (
			this.altPressed &&
			e.cancelable &&
			this._options.preventAltDefault
		) {
			e.preventDefault();
		}
	...
	private static _onMouseDown = (e: MouseEvent): void => {
		if (!this._mouseButtons.has(e.button)) {
			...
			this._mouseButtonDragOrigin.set(e.button, {
				...this._mousePosition,
			});
			...
			this._mouseButtonClickCount.set(
				e.button,
				now - lastTime < 300 ? count + 1 : 1,
			);
	...
	private static _onWheel = (e: WheelEvent): void => {
		const shouldPreventDefault =
			this._options.preventWheelDefault || e.ctrlKey;
