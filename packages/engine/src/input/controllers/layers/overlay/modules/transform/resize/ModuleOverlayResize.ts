...
export class ModuleOverlayTransformResize implements IOverlayTransformSubModule {
	...
	private _updateResizeFromInput(): void {
		...
		const worldDelta = this._subtractPoints(
			currentWorldPoint,
			this._resizeStartWorldPoint,
		);
		...
		const offsetX = anchorIsRightX
			? -Math.max(rawWidth, 0)
			: Math.min(rawWidth, 0);
		const offsetY = anchorIsBottomY
			? -Math.max(rawHeight, 0)
			: Math.min(rawHeight, 0);
	...
	private _hitTestHandleScreen(
		entry: AxisHandleEntry,
		screenPoint: Point,
	): boolean {
		if (!this._context) {
			return false;
		...
		if (this._isEdgeAxis(axis)) {
			...
			return (
				this._distanceToSegment(screenPoint, p0, p1) <=
				strokeWidth * 0.5
			);
		}
	...
	private _getEdgeHitStrokeWidth(
		handle: ResizeHandle,
		axis: ResizeEdgeAxis,
	): number {
		if (axis === "n" || axis === "s") {
