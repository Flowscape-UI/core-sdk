...
export class ModuleOverlayTransformMove implements IOverlayTransformSubModule {
	...
	private _updateMoveState(): void {
		...
		if (
			Math.hypot(dx, dy) >=
			ModuleOverlayTransformMove.DRAG_START_THRESHOLD_PX
		) {
			this._hasMoved = true;
		}
	}
	...
	private _updateMoveFromInput(): void {
		...
		if (
			!this._isMoving ||
			!this._moveStartWorldPoint ||
			!this._moveStartNodePosition
		) {
			return;
		}
	...
	private _getHandle(): IHandleTransformPosition | null {
		...
		const handle =
			this._context.overlay.transformHandleManager.getById(
				"transform-position",
			);
	...
	private _isTransformPositionHandle(
		value: unknown,
	): value is IHandleTransformPosition {
