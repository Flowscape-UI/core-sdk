...
export class ModuleOverlayTransform implements IInputModule<OverlayInputContext> {
	...
	private readonly _moduleManager =
		new ModuleManager<IOverlayTransformSubModule>();
	...
	public update(): void {
		...
		if (activeModule) {
			...
			if (!activeModule.isActive()) {
				...
				const clickSelectNode =
					this._consumeMoveClickSelectNode(activeModule);

				if (clickSelectNode) {
					const modulesChanged =
						this._setNodeForAllModules(clickSelectNode);
					const transformChanged =
						this._setNodeForTransformHandle(clickSelectNode);
		...
		if (hoveredNode && currentNodeId !== hoveredNode.id) {
			...
			const transformChanged =
				this._setNodeForTransformHandle(hoveredNode);
	...
	private _consumeMoveClickSelectNode(
		module: IOverlayTransformSubModule,
	): IShapeBase | null {
		if (module.id !== "overlay-transform-move") {
			return null;
		}
