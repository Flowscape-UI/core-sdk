...
export class RendererCanvasRegistry {
	private readonly _renderers = new Map<NodeType, IRendererNodeCanvas>();
	...
	public register<TNode extends INode>(
		type: NodeType,
		renderer: IRendererNodeCanvas<TNode>,
	): void {
		if (this._renderers.has(type)) {
			throw new Error(
				`Renderer for node type "${type}" is already registered.`,
			);
		}

		this._renderers.set(type, renderer as IRendererNodeCanvas);
	}
	...
	public get<TNode extends INode>(
		type: NodeType,
	): IRendererNodeCanvas<TNode> | undefined {
		return this._renderers.get(type) as
			IRendererNodeCanvas<TNode> | undefined;
	}
	...
	public getOrThrow<TNode extends INode>(
		type: NodeType,
	): IRendererNodeCanvas<TNode> {
		const renderer = this.get<TNode>(type);

		if (!renderer) {
			throw new Error(
				`Renderer for node type "${type}" is not registered.`,
			);
		}
