...
import { type ShapePathCommand, type NodePath } from "../../../../nodes";
...
export class RendererCanvasPath extends RendererCanvasBase<NodePath> {
	public create(node: NodePath): Konva.Group {
		...
		const fill = new Konva.Shape({
			...
			listening: false,
			sceneFunc: (ctx, shape) => {
				const commands = (shape.getAttr("pathCommands") ??
					[]) as readonly ShapePathCommand[];
	...
	protected onUpdate(node: NodePath, view: Konva.Group): void {
		...
		fill.setAttrs({
			...
			strokeColor:
				"getStrokeFill" in node
					? (node as any).getStrokeFill()
					: undefined,
			strokeWidth:
				"getStrokeWidth" in node ? (node as any).getStrokeWidth() : 0,
		});
