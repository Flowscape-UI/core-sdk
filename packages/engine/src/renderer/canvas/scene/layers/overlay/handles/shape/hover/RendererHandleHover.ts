...
export class RendererHandleHoverCanvas extends RendererHandleBase<IHandleHover> {
	...
	protected override _onUpdate(
		target: RendererHandleTarget<IHandleHover>,
	): void {
	...
	private _extractScreenLineSegments(
		...
		for (const command of commands) {
			if (command.type === "moveTo") {
				cursor = this._toScreenPoint(
					this._applyMatrix(command.point, matrix),
				);
				continue;
			}

			if (command.type === "lineTo") {
				const next = this._toScreenPoint(
					this._applyMatrix(command.point, matrix),
				);
	...
	private _updateTextDebug(handle: IHandleHover, node: NodeText): void {
		...
		const matrix = node.getWorldMatrix();
		const strokeDash =
			handle.getDebugStrokeType() === HandleDebugDrawType.Dashed
				? [6, 4]
				: [];
		...
		for (let i = 0; i < boxes.length; i += 1) {
			...
			const p1 = this._toScreenPoint(
				this._applyMatrix({ x: box.x, y: box.y }, matrix),
			);
			const p2 = this._toScreenPoint(
				this._applyMatrix({ x: box.x + box.width, y: box.y }, matrix),
			);
			const p3 = this._toScreenPoint(
				this._applyMatrix(
					{ x: box.x + box.width, y: box.y + box.height },
					matrix,
				),
			);
			const p4 = this._toScreenPoint(
				this._applyMatrix({ x: box.x, y: box.y + box.height }, matrix),
			);
	...
	private _createHatchDebugShape(): Konva.Shape {
		return new Konva.Shape({
			...
			sceneFunc: (ctx, shape) => {
				...
				const fillType = (shape.getAttr("debugFillType") ??
					HandleDebugDrawType.Solid) as HandleDebugDrawType;
				...
				const hatchColor =
					(shape.getAttr("hatchColor") as string | undefined) ??
					shape.stroke() ??
					"#4DA3FF";
				const hatchWidth = Math.max(
					1,
					(shape.getAttr("hatchWidth") as number | undefined) ?? 1,
				);
				...
				for (
					let x = bounds.minX - diagonal;
					x <= bounds.maxX + diagonal;
					x += hatchSpacing
				) {
					ctx.beginPath();
	...
	private _getPointsBounds(points: readonly number[]): {
		minX: number;
		minY: number;
		maxX: number;
		maxY: number;
