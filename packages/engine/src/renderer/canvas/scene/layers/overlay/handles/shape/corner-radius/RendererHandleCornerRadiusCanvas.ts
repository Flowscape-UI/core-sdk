...
export class RendererHandleCornerRadiusCanvas extends RendererHandleBase<IHandleCornerRadius> {
	...
	protected override _onUpdate(
		target: RendererHandleTarget<IHandleCornerRadius>,
	): void {
		...
		const viewRadius =
			Math.max(handle.getWidth(), handle.getHeight()) * 0.5;
		const hitRadius =
			Math.max(
				this._resolveHitWidth(handle),
				this._resolveHitHeight(handle),
			) * 0.5;
	...
	private _updateDebugViews(handle: IHandleCornerRadius): void {
		...
		if (
			!this._debugSectionView ||
			!this._debugDiagonalView ||
			!this._debugOriginView
		) {
		...
		const strokeDash =
			handle.getDebugStrokeType() === "dashed" ? [4, 4] : [];
