...
export class LayerOverlayTransformRenderersManager {
	...
	public attach(overlay: ILayerOverlay): void {
		...
		const pivotHandle = overlay.transformHandleManager.getById(
			"transform-pivot",
		) as IHandleTransformPivot | null;
		...
		if (pivotHandle) {
			...
			this._pivotRenderer
				.getRoot()
				.setAttr("overlayId", "transform-pivot");
			this._pivotRenderer
				.getRoot()
				.setAttr("overlayZ", pivotHandle.getZIndex());
		} else {
	...
	private _rebuildResizeRenderers(): void {
		...
		for (const id of RESIZE_EDGE_IDS) {
			...
			renderer
				...
				.setAttr(
					"overlayZ",
					(handle as IHandleTransformResizeEdge).getZIndex(),
				);
		}
		...
		for (const id of RESIZE_VERTEX_IDS) {
			...
			renderer
				...
				.setAttr(
					"overlayZ",
					(handle as IHandleTransformResizeVertex).getZIndex(),
				);
		}
	}
	...
	private _rebuildRotateRenderers(): void {
		...
		for (const id of ROTATE_IDS) {
			...
			renderer
				...
				.setAttr(
					"overlayZ",
					(handle as IHandleTransformRotate).getZIndex(),
				);
		}
	}
