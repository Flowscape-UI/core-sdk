...
export class LayerOverlayFreeRenderersManager {
	...
	public add(id: ID, renderer: IRendererHandleBase<IHandleBase>): void {
		if (this._renderers.has(id)) {
			throw new Error(
				`Overlay free renderer with id "${id}" is already added.`,
			);
		}
	...
	private _attachRenderer(
		id: ID,
		renderer: IRendererHandleBase<IHandleBase>,
	): void {
		if (!this._overlay || !this._camera) {
			return;
		}

		const handle = this._overlay.freeHandleManager.getById(
			id,
		) as IHandleBase | null;
		...
		renderer.attach(new RendererHandleTarget(handle, this._camera));
	}
}
