...
export abstract class RendererHandleBase<
	T extends IHandleBase,
> implements IRendererHandleBase<T> {
	...
	private _tryGetPathShape(node: IShapeBase): RectPathShape | null {
		if (
			!("toPathCommands" in node) ||
			typeof (node as { toPathCommands?: unknown }).toPathCommands !==
				"function"
		) {
			return null;
		}
		...
		const commands = (
			node as { toPathCommands(): unknown }
		).toPathCommands();
	...
	private _pathToWorldPoints(shape: RectPathShape): Point[] {
		...
		for (const command of shape.commands) {
			if (command.type === "moveTo" || command.type === "lineTo") {
				points.push(
					this._applyMatrix(command.point, shape.worldMatrix),
				);
				continue;
			}
			...
			if (command.type === "arcTo") {
				points.push(
					...this._arcToWorldPoints(command, shape.worldMatrix),
				);
			}
		}
	...
	private _arcToWorldPoints(
		command: Extract<ShapePathCommand, { type: "arcTo" }>,
		matrix: Matrix,
	): Point[] {
