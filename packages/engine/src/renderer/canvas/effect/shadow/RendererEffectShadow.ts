...
export class RendererEffectShadow {
	...
	constructor(effect: EffectShadow, node: Konva.Node) {
	...
	public update(): void {
		...
		if (mode === ShadowMode.Cutout) {
			this._cutoutShape.setAttrs({
				...
				radiusBottomRight:
					this._shadowShape.getAttr("radiusBottomRight"),
		...
		if (mode === ShadowMode.Cutout) {
			...
			const padding =
				Math.max(4, Math.ceil(blur * 3)) + Math.max(0, spread);
			...
			this._group.cache({
				...
				height: Math.max(
					1,
					Math.ceil(height + spread * 2 + padding * 2),
				),
			});
		}
	}
