...
export abstract class BaseRendererHost implements IRendererHost {
	...
	protected _onAttachBindings(
		scene: IScene,
		callback: (binding: LayerBinding) => void,
	): void {
		if (this._scene) {
			this.detach();
		}
		...
		scene.getLayerRendererBindings().forEach((binding) => {
			...
			const typeMatch = this.type === null || renderer.type === this.type;

			const idMatch = this.id === -1 || renderer.id === this.id;
