...
export class Camera implements ICamera {
	...
	/***************************************************************************/
	public onChange(
		callback: (state: Readonly<CameraState>) => void,
	): () => void {
	...
	public worldToScreen(world: Point): Point {
		return this._worldToScreenWithState(world, this._viewport, {
			x: this._x,
			y: this._y,
			scale: this._scale,
			rotation: this._rotation,
	...
	public screenToWorld(screen: Point): Point {
		return this._screenToWorldWithState(screen, this._viewport, {
			x: this._x,
			y: this._y,
			scale: this._scale,
			rotation: this._rotation,
	...
	public setLimits(minScale: number, maxScale: number): void {
		...
		if (this._minScale === newMinScale && this._maxScale === newMaxScale) {
			return;
		}
	...
	public setScale(scale: number): void {
		this._update({
			scale: MathF32.clamp(scale, this._minScale, this._maxScale),
		});
	}
	...
	public panByScreen(dx: number, dy: number): void {
		...
		const { x: rdx, y: rdy } = this._rotate(
			{ x: wx, y: wy },
			this._rotation,
		);
		this._update({
	...
	public zoomAtScreen(screenPoint: Point, factor: number): void {
		...
		const after = this._screenToWorldWithState(
			screenPoint,
			this._viewport,
			nextState,
		);
	...
	public fitToRect(
		rect: Rect,
		options?: { padding?: number; resetRotation?: boolean },
	): void {
		...
		if (rotation === 0) {
			...
			const rotatedWidth =
				this._viewport.width * cos + this._viewport.height * sin;
			const rotatedHeight =
				this._viewport.width * sin + this._viewport.height * cos;
	...
	protected _worldToScreenWithState(
		...
		// 3) scale
		const scaled = {
			x: rot.x * cameraState.scale,
			y: rot.y * cameraState.scale,
		};
	...
	protected _screenToWorldWithState(
		...
		// 2) unscale
		const unscaled = {
			x: rel.x / cameraState.scale,
			y: rel.y / cameraState.scale,
		};
