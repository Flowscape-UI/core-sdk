...
export class ModuleRulerUI extends ModuleBaseLayerUI implements IModuleRulerUI {
	...
	private readonly _guideElementsHorizontal: Map<number, HTMLDivElement> =
		new Map();
	private readonly _guideElementsVertical: Map<number, HTMLDivElement> =
		new Map();
	...
	public getMinTickPx(): number {
		return this._minTickPx;
	}
	...
	public getMinMajorTickPx(): number {
		return this._minMajorTickPx;
	}
	...
	public override attach(root: HTMLElement): void {
		...
		this._topWrap?.addEventListener("pointerdown", this._onTopPointerDown, {
			passive: false,
		});
		this._leftWrap?.addEventListener(
			"pointerdown",
			this._onLeftPointerDown,
			{ passive: false },
		);
	...
	public override detach(): void {
		this._topWrap?.removeEventListener(
			"pointerdown",
			this._onTopPointerDown,
		);
		this._leftWrap?.removeEventListener(
			"pointerdown",
			this._onLeftPointerDown,
		);
	...
	private _onWindowPointerMove = (e: PointerEvent): void => {
		...
		if (
			...
			this._activeDraggedGuideAxis
		) {
			const axis = this._activeDraggedGuideAxis;
			const guide = this._getGuideByAxisAndId(
				axis,
				this._activeDraggedGuideId,
			);

			if (guide) {
				if (axis === "vertical") {
					guide.setValue(
						cam.screenToWorld({ x: e.clientX - rect.left, y: 0 }).x,
					);
				} else {
					guide.setValue(
						cam.screenToWorld({ x: 0, y: e.clientY - rect.top }).y,
					);
				}
				this._syncGuideElements();
			}
	...
	private _onWindowKeyDown = (e: KeyboardEvent): void => {
		...
		if (this._focusedGuideId === null || this._focusedGuideAxis === null)
			return;
	...
	private _addGuide(axis: "horizontal" | "vertical", value: number): boolean {
		...
		const elements =
			axis === "horizontal"
				? this._guideElementsHorizontal
				: this._guideElementsVertical;
	...
	private _focusGuide(axis: "horizontal" | "vertical", id: number): void {
		if (this._focusedGuideId === id && this._focusedGuideAxis === axis)
			return;
	...
	private _clearFocusedGuide(): void {
		...
		const guide = this._getGuideByAxisAndId(
			this._focusedGuideAxis,
			this._focusedGuideId,
		);
	...
	private _getGuideByAxisAndId(
		axis: "horizontal" | "vertical",
		id: number,
	): IRulerGuideLine | null {
	...
	private _cancelActiveGuideDrag(): void {
		if (this._activeGuideOwner && this._activeGuidePointerId !== null) {
			try {
				this._activeGuideOwner.releasePointerCapture(
					this._activeGuidePointerId,
				);
			} catch {}
		}
	...
	private _applyGuideElementStyles(
		el: HTMLDivElement,
		axis: "horizontal" | "vertical",
		guide: IRulerGuideLine,
	): void {
	...
	private _getRulerStepWorld(scale: number): number {
		const targetWorld =
			Math.max(this._minLabelPx, 60) / Math.max(scale, 0.000001);
		const steps = [
			1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000,
			50000,
		];
		return (
			steps.find((s) => s >= targetWorld) ??
